  ...
  return (
    <div className="space-y-5">
      ...
      {visitStatus !== "CANCELLED" && (
        <div className="border-2 border-dashed border-pk-border rounded-pk-lg p-6 text-center">
          <svg className="w-10 h-10 text-pk-text-muted mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          <p className="text-sm text-pk-text-muted mb-3">Click to upload or drag &amp; drop files</p>
          <div className="flex items-center justify-center gap-3">
            ...
            <label className="cursor-pointer bg-pk-teal-600 text-white px-4 py-2 rounded-pk-sm text-sm font-medium hover:bg-pk-teal-700 transition">
              Browse File
      ...
      {attachments.length === 0 ? (
        <p className="text-center text-pk-text-muted text-sm py-4">No attachments yet</p>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {attachments.map((att) => (
            <div key={att.id} className="border border-pk-border rounded-pk-lg p-3 text-center relative group">
              {att.mimeType.startsWith("image/") ? (
                <a href={att.fileUrl} target="_blank" rel="noreferrer">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={att.fileUrl} alt={att.originalName} className="w-full h-24 object-cover rounded-pk-sm mb-2" />
                </a>
              ) : (
                <div className="w-full h-24 bg-pk-surface-sunken rounded-pk-sm flex items-center justify-center mb-2">
                  <svg className="w-8 h-8 text-pk-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                </div>
              )}
              <p className="text-xs font-medium text-pk-text-secondary truncate">{att.originalName}</p>
