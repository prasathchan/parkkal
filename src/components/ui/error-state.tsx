...
export function ErrorState({ message = "Something went wrong. Please try again.", onRetry }: ErrorStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
      ...
      <p className="text-sm font-medium text-pk-text-secondary mb-1">Failed to load</p>
      <p className="text-xs text-pk-text-muted max-w-xs mb-4">{message}</p>
      {onRetry && (
        <button
          onClick={onRetry}
          className="text-xs text-pk-teal-600 hover:text-pk-teal-800 font-medium underline"
        >
          Try again
        </button>
      )}
