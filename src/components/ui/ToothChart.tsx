...
const CONDITION_COLORS: Record<ToothCondition, { bg: string; border: string; text: string }> = {
  HEALTHY:    { bg: "#FFFFFF", border: "#B0A99B", text: "#847D6E" },
  CARIES:     { bg: "#C0392B", border: "#C0392B", text: "#FFFFFF" },
  FILLING:    { bg: "#0B6E6E", border: "#0B6E6E", text: "#FFFFFF" },
  CROWN:      { bg: "#C8873A", border: "#C8873A", text: "#FFFFFF" },
  MISSING:    { bg: "#C4BDB0", border: "#B0A99B", text: "#4A4439" },
  ROOT_CANAL: { bg: "#6B4A2F", border: "#6B4A2F", text: "#FFFFFF" },
  BRIDGE:     { bg: "#0B5654", border: "#0B5654", text: "#FFFFFF" },
  IMPLANT:    { bg: "#6E7B7E", border: "#6E7B7E", text: "#FFFFFF" },
  FRACTURED:  { bg: "#B35B43", border: "#B35B43", text: "#FFFFFF" },
  WATCH:      { bg: "#FCEFD6", border: "#B8770F", text: "#9A5B0A" },
};
...
// FDI layout — upper and lower rows from patient's right to left (chart view)
...
function ToothCell({
  ...
  // Sync notes field when data prop changes (e.g. after external save)
  useEffect(() => {
    if (!open) setEditNotes(data?.notes ?? "");
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data?.notes, open]);

  // Close on outside click — discard pending change
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div ref={ref} className="relative flex flex-col items-center">
      ...
      */}
      ...
      >
        {/* Glyph overlays — colour-blind-safe cues */}
      ...
      {/* Condition popover — position:fixed escapes overflow-x:auto clipping */}
      {open && popoverPos && (
        <div
          style={{
            position: "fixed",
        ...
        >
          <p className="text-xs font-semibold text-pk-text-secondary px-1 pb-1 border-b border-pk-border mb-1">
            ...
            {condition !== "HEALTHY" && (
              <span
                ...
                style={{ background: CONDITION_COLORS[condition].bg, color: CONDITION_COLORS[condition].text }}
              >
          ...
          {linkedTreatment && (
            <div className="flex items-center gap-1.5 text-xs text-pk-teal-700 bg-pk-teal-50 rounded px-2 py-1.5 mb-2">
          ...
          <div className="grid grid-cols-2 gap-1 mb-2">
            {ALL_CONDITIONS.map((c) => {
              ...
              const isSelected = displayCondition === c;
              const isCurrent  = condition === c;
              return (
                <button
                  key={c}
                  type="button"
                  ...
                  className={`text-left text-xs px-2 py-1 rounded-pk-sm border transition-all ${
                    isSelected
                      ? "ring-2 ring-offset-1 ring-[var(--pk-accent)]"
                      : isCurrent
          ...
          />
          {/* Confirm / Undo — appears only when something has changed */}
          {hasChange && (
          ...
          {!hasChange && (
            <button
              ...
              className="mt-2 w-full border border-pk-border text-pk-text-secondary text-xs py-1.5 rounded-pk-sm font-medium hover:bg-pk-surface-raised transition"
            >
              Close
            </button>
          )}
...
export function ToothChart({ data, readOnly = false, onChange, highlightTeeth, linkedTreatments, onToothChange, onToothSelect }: ToothChartProps) {
  ...
  // Shared renderer for a row of teeth — avoids duplicating the map logic
  function renderRow(teeth: number[], isUpper: boolean) {
    return teeth.map((n, i) => (
      <React.Fragment key={n}>
        {/* Midline gap between Q1/Q2 (upper) and Q4/Q3 (lower) */}
        {i === 8 && <div className="w-2 flex-shrink-0" aria-hidden="true" />}
        <ToothCell
          number={n}
          data={data[String(n)]}
          isUpper={isUpper}
