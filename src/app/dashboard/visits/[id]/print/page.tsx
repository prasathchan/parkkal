      ...
      <div className="print-container">
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
              {clinic.logoUrl ? (
                ...
                <div style={{ width: 36, height: 36, background: "#2563eb", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 20, fontWeight: "bold", color: "#1e293b" }}>{clinic.name}</span>
            </div>
            <p style={{ fontSize: 12, color: "#64748b", margin: "2px 0" }}>{formatAddressDisplay(parseAddress(clinic.address))}</p>
          ...
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            ...
            <p style={{ fontSize: 12, color: "#64748b", margin: "4px 0 0" }}>#{visit.visitCode}</p>
            <p style={{ fontSize: 12, color: "#64748b", margin: "2px 0 0" }}>{formatDate(visit.visitDate)}</p>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          <p style={{ fontWeight: "bold", fontSize: 13, marginBottom: 8, borderBottom: "1px solid #e2e8f0", paddingBottom: 4 }}>
            SERVICES RENDERED
          </p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, fontSize: 13, maxWidth: 320, marginLeft: "auto" }}>
            {gst ? (
              <>
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ color: "#64748b" }}>CGST @ {clinic.cgstRate}%:</span>
                  <span>{formatCurrency(gst.cgstAmt)}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ color: "#64748b" }}>SGST @ {clinic.sgstRate}%:</span>
                  <span>{formatCurrency(gst.sgstAmt)}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid #e2e8f0", paddingTop: 5, fontWeight: "bold" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <span style={{ color: "#16a34a", fontWeight: "bold" }}>{formatCurrency(visit.paidAmount)}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid #e2e8f0", paddingTop: 5, fontWeight: "bold" }}>
              <span>Balance due:</span>
              <span style={{ color: due > 0 ? "#dc2626" : "#64748b" }}>{formatCurrency(due)}</span>
            </div>
          </div>
        </div>
            ...
            {prescriptions.map((rx) => {
              ...
              return (
                <div key={rx.id} style={{ marginBottom: 12 }}>
                  ...
                  {rx.instructions && (
                    <p style={{ marginTop: 6, color: "#475569" }}><strong>Instructions:</strong> {rx.instructions}</p>
                  )}
                </div>
              );
            })}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40, marginTop: 40, fontSize: 13 }}>
          <div>
            <div style={{ borderTop: "1px solid #374151", paddingTop: 8, marginTop: 32 }}>
              ...
              <p style={{ color: "#64748b", margin: "4px 0 0" }}>{formatDoctorName(visit.doctorName)}</p>
            </div>
          </div>
          <div>
            <div style={{ borderTop: "1px solid #374151", paddingTop: 8, marginTop: 32 }}>
              <p style={{ margin: 0 }}>Patient Signature</p>
              <p style={{ color: "#64748b", margin: "4px 0 0" }}>{visit.patientName}</p>
            </div>
          </div>
        </div>
