...
function SlideoverForm({ role, onClose, onSave }: SlideoverFormProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-md bg-pk-surface shadow-pk-e3 flex flex-col overflow-y-auto">
        ...
        <div className="flex-1 px-6 py-5 space-y-5">
          {role && role.userCount > 0 && (
            <div className="bg-pk-warning-fill border border-pk-warning-border rounded-pk-sm px-4 py-3 flex items-start gap-2">
          ...
          {error && (
            <div className="bg-pk-danger-fill border border-pk-danger-border rounded-pk-sm px-4 py-3 text-sm text-pk-danger-text">
              {error}
            </div>
          )}
        ...
        <div className="px-6 py-4 border-t border-pk-border flex gap-3">
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 bg-pk-teal-600 hover:bg-pk-teal-700 text-white text-sm font-medium px-4 py-2.5 rounded-pk-sm transition disabled:opacity-60"
          >
            {saving ? "Saving..." : role ? "Save Changes" : "Create Role"}
          </button>
          <button
            onClick={onClose}
            className="px-4 py-2.5 border border-pk-border-strong text-pk-text-secondary text-sm font-medium rounded-pk-sm hover:bg-pk-surface-raised transition"
          >
            Cancel
          </button>
        </div>
...
function DeleteModal({ role, allRoles, onClose, onDeleted }: DeleteModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-pk-surface rounded-pk-lg shadow-pk-e3 w-full max-w-md">
            ...
            <>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 bg-pk-danger hover:bg-pk-danger text-white text-sm font-medium px-4 py-2.5 rounded-pk-sm transition disabled:opacity-60"
                >
                  {loading ? "Migrating..." : `Migrate ${role.userCount} member${role.userCount !== 1 ? "s" : ""} & Delete`}
                </button>
                <button onClick={onClose} className="px-4 py-2.5 border border-pk-border-strong text-pk-text-secondary text-sm font-medium rounded-pk-sm hover:bg-pk-surface-raised transition">
                  Cancel
                </button>
              </div>
            </>
      ...
      <main id="main-content" className="flex-1 p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-pk-teal-600 hover:bg-pk-teal-700 text-white text-sm font-medium px-4 py-2.5 rounded-pk-sm transition"
          >
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1,2,3,4,5,6].map((i) => (
              <div key={i} className="bg-pk-surface rounded-pk-lg border border-pk-border p-5 animate-pulse h-48" />
            ))}
          </div>
        ) : roles.length === 0 ? (
          <div className="text-center py-16">
            ...
            <button
              onClick={() => { setEditingRole(null); setShowForm(true); }}
              className="inline-flex items-center gap-2 bg-pk-teal-600 hover:bg-pk-teal-700 text-white text-sm font-medium px-4 py-2 rounded-pk-sm transition"
            >
              + New Role
            </button>
          </div>
...
function RoleCard({ role, onEdit, onDelete }: RoleCardProps) {
  ...
  return (
    <div className="bg-pk-surface rounded-pk-lg border border-pk-border shadow-pk-e1 hover:shadow-pk-e2 transition p-5 flex flex-col gap-4">
      ...
      <div className="flex items-center gap-2 pt-1 border-t border-pk-border">
        <button
          onClick={onEdit}
          className="flex items-center gap-1.5 text-xs text-pk-text-secondary hover:text-pk-teal-600 hover:bg-pk-teal-50 px-2.5 py-1.5 rounded-pk-sm transition"
        >
        ...
        {role.isSystem === 1 ? (
          <span
            ...
            className="flex items-center gap-1.5 text-xs text-pk-text-muted px-2.5 py-1.5 rounded-pk-sm cursor-not-allowed"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <button
            onClick={onDelete}
            className="flex items-center gap-1.5 text-xs text-pk-text-secondary hover:text-pk-danger-text hover:bg-pk-danger-fill px-2.5 py-1.5 rounded-pk-sm transition"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
            </svg>
