  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">View all organisations. Manually activate cash-paying clinics here.</p>
      </div>
      ...
      {activeForm && (
        <div className="bg-white rounded-xl border border-blue-200 p-5 space-y-4">
          <h2 className="font-semibold text-slate-900 text-sm">Manually activate subscription</h2>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setActiveForm(null)}
              className="px-4 py-1.5 rounded-lg text-sm border border-slate-200 text-slate-600 hover:bg-slate-50 transition">
              Cancel
            </button>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {subs.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-5 py-3 text-slate-600">{s.planName} <span className="text-xs text-slate-400">₹{s.priceMonthly}/mo</span></td>
                    <td className="px-5 py-3">
                    ...
                    <td className="px-5 py-3 text-right">
                      {s.daysRemaining !== null ? (
                        <span className={s.daysRemaining < 0 ? "text-red-600 font-medium" : s.daysRemaining <= 7 ? "text-yellow-600" : "text-slate-600"}>
                          {s.daysRemaining < 0 ? `${Math.abs(s.daysRemaining)}d overdue` : `${s.daysRemaining}d`}
                        </span>
                      ) : "—"}
                    </td>
                    <td className="px-5 py-3 text-xs text-slate-400 max-w-[200px] truncate">{s.notes ?? "—"}</td>
                    <td className="px-5 py-3 text-right">
                      <button
