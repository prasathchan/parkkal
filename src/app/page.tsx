...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-slate-100">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-bold text-lg text-slate-900">Parkkal</span>
          </div>
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-slate-600">
            <a href="#features" className="hover:text-slate-900 transition">Features</a>
            <a href="#pricing" className="hover:text-slate-900 transition">Pricing</a>
            <a href="mailto:support@parkkal.com" className="hover:text-slate-900 transition">Contact</a>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/login" className="text-sm font-medium text-slate-600 hover:text-slate-900 transition">Sign in</Link>
            <Link href="/signup" className="bg-blue-600 text-white text-sm font-semibold px-4 py-2 rounded-lg hover:bg-blue-700 transition">
              Start free trial
            </Link>
          </div>
        </div>
      ...
      {/* Hero */}
      <section className="bg-gradient-to-br from-blue-600 via-blue-700 to-blue-900 text-white py-24 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-blue-500/30 border border-blue-400/30 rounded-full px-4 py-1.5 text-sm font-medium mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/signup"
              className="inline-flex items-center justify-center gap-2 bg-white text-blue-700 font-bold text-base px-8 py-3.5 rounded-xl hover:bg-blue-50 transition shadow-lg">
          ...
          <p className="text-blue-200 text-sm mt-6">No credit card required · Cancel anytime</p>
        </div>
      </section>
      ...
      <section id="features" className="py-20 px-6 bg-slate-50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-slate-500 text-lg max-w-xl mx-auto">Designed from the ground up for dental practices in India — not adapted from a generic template.</p>
          </div>
      ...
      {/* Compliance callout */}
      <section className="py-14 px-6 bg-blue-600 text-white">
      ...
      <section id="pricing" className="py-20 px-6 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-slate-500 text-lg">All plans include a 14-day free trial. No hidden fees.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
            {plans.map((p) => (
              <div key={p.name} className={`rounded-2xl border p-8 flex flex-col ${p.highlight ? "border-blue-500 ring-2 ring-blue-500 bg-blue-50" : "border-slate-200"}`}>
                {p.highlight && (
                ...
                <div className="mb-6">
                  ...
                  {p.price !== "Custom" && <span className="text-slate-400 text-sm ml-1">/month + GST</span>}
                </div>
                ...
                {p.price === "Custom" ? (
                  <a href="mailto:support@parkkal.com?subject=Enterprise%20enquiry"
                    className="block text-center border border-slate-300 text-slate-700 font-semibold py-3 rounded-xl hover:bg-slate-50 transition text-sm">
                    Contact us
                  </a>
                ) : (
                  <Link href="/signup"
                    className={`block text-center font-semibold py-3 rounded-xl text-sm transition ${p.highlight ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-slate-900 text-white hover:bg-slate-800"}`}>
                    Start free trial
                  </Link>
                )}
              </div>
      ...
      <section className="py-20 px-6 bg-slate-900 text-white">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-slate-400 text-lg mb-8">Join dental clinics across India who trust Parkkal to run their practice.</p>
          <Link href="/signup"
            className="inline-flex items-center gap-2 bg-blue-600 text-white font-bold text-base px-8 py-3.5 rounded-xl hover:bg-blue-700 transition">
      ...
      <footer className="bg-slate-950 text-slate-400 py-10 px-6">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-300">Parkkal</span>
          </div>
          <div className="flex gap-6 text-sm">
            <Link href="/login" className="hover:text-slate-200 transition">Sign in</Link>
            <Link href="/signup" className="hover:text-slate-200 transition">Sign up</Link>
            <a href="mailto:support@parkkal.com" className="hover:text-slate-200 transition">Support</a>
          </div>
